...
def _find_genre_python_generator(genre: str, system: str) -> Any | None:
    ...
    The file must define a function `emit_system(slice_data, hlr, spec) -> str`
...
def _pool_names_from_imap(imap: ImpactMap, hlt_roles: dict | None) -> list[str]:
    """Pool names the LLM should know about for THIS game. Delegates to
    scene_gen so both the emitter and the LLM prompt share one view.
    """
    from . import scene_gen
    return [scene_gen.pool_name_for(o)
            for o in scene_gen.pool_owners_from_imap(imap, hlt_roles)]
...
async def generate_all_systems(
    ...
    pool_names = _pool_names_from_imap(imap, hlt_roles)
        ...
        async with sem:
            try:
                return await _emit_one_system(
                    ...
                    constants, pool_names, system_descriptions,
                )
            except Exception as exc:
...
def generate_all_systems_sync(
    ...
    return asyncio.run(generate_all_systems(imap, hlr, output_dir, constants, concurrency))
